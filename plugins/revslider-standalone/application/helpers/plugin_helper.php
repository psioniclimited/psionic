...
if( ! function_exists('is_plugin_active')) {
	...
	function is_plugin_active($plugin) {
		...
		return $ci->plugin->isPluginActive($plugin);
	}
...
if( ! function_exists('activate_plugin')) {
	...
	function activate_plugin($plugin) {
		$ci = &get_instance();
		$ci->load->library('plugin');
		return $ci->plugin->activatePlugin($plugin);
	}
...
if( ! function_exists('deactivate_plugins')) {
	...
	function deactivate_plugins($plugin) {
		$ci = &get_instance();
		$ci->load->library('plugin');
		return $ci->plugin->deactivatePlugin($plugin);
	}
...
if( ! function_exists('plugins_url')) {
	...
	function plugins_url($file, $plugin) {
        return base_url() . WP_PLUGIN_DIR . basename($plugin, '.php') . '/' . $file;
	}
...
if( ! function_exists('plugin_dir_path')) {
	...
	function plugin_dir_path($plugin) {
        return FCPATH . WP_PLUGIN_DIR . basename($plugin, '.php') . '/';
	}
