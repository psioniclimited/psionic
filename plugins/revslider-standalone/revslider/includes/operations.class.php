...
class RevSliderOperations extends RevSliderElementsBase{
	...
	<head>
        ...
        <?php if ($slider->getParam('wb_enable', 'off') == 'on') : ?>
            <script type="text/javascript" src="<?php echo $path_js; ?>revolution.addon.whiteboard.min.js"></script>
        <?php endif; ?>
...
ob_end_clean();
		...
		if($export_real){ //only include if real export
            ...
            if(!$usepcl){
                ...
                if ($slider->getParam('wb_enable', 'off') == 'on') {
                    $zip->addFile(WHITEBOARD_PLUGIN_PATH.'public/assets/js/revolution.addon.whiteboard.min.js', PCLZIP_OPT_REMOVE_PATH, WHITEBOARD_PLUGIN_PATH.'public/assets/js/', PCLZIP_OPT_ADD_PATH, 'js/');
                }
...
$presets[] = array (
    ...
    public static function create_slider_page($added){
        ...
        foreach($added as $sid){
            ...
            $alias = $slider->getAlias();
            if($alias !== ''){
                $content = 'RevSliderEmbedder::putRevSlider(\'' . $alias . '\');' . "\n" . $content; //this way we will reorder as last comes first
            }
        }
